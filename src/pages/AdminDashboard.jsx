...
const AdminDashboard = () => {
  ...
  const [newProduct, setNewProduct] = useState({ 
    name: '', size: '', price: '', type: 'stock', category: tenant.categories[0]?.name || 'Adulto', is_favorite: false, image: null, stock_quantity: 0, stock_by_size: {} 
  });
  ...
  const [openCategory, setOpenCategory] = useState(tenant.categories[0]?.name || 'Adulto');
  ...
  const [availableSizes, setAvailableSizes] = useState(
    tenant.sizes_by_category || { Adulto: ['S', 'M', 'L', 'XL'], Niño: ['2', '4', '6', '8', '10', '12', '14', '16'] }
  );

  useEffect(() => {
    // If sizes change, update tenant on the backend
    if (isAuthenticated) {
      fetch(`/api/tenants`, {
        method: 'PUT',
  ...
  }, [availableSizes, isAuthenticated, tenant.slug]);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && !loading && customers.length > 0) {
      ...
      if (!exists) {
        fetch(`/api/customers?tenant_id=${tenant.slug}`, { 
  ...
  const fetchProducts = async () => {
    try { 
      const r = await fetch(`/api/products?tenant_id=${tenant.slug}`); 
      if (!r.ok) throw new Error('Network error');
      setProducts(await r.json()); 
    } catch(e) {} 
  };
  const fetchOrdersHistory = async () => {
    try { const r = await fetch(`/api/orders?tenant_id=${tenant.slug}`); setOrdersHistory(await r.json()); } catch(e) { console.error(e); }
  };
  const fetchCustomers = async () => {
    try { const r = await fetch(`/api/customers?tenant_id=${tenant.slug}`); setCustomers(await r.json()); } catch(e) { console.error(e); }
  };
  const fetchSales = async () => {
    try { const r = await fetch(`/api/sales?tenant_id=${tenant.slug}`); setSales(await r.json()); } catch(e) { console.error(e); }
  };
  const fetchReservations = async () => {
    try { const r = await fetch(`/api/reservations?tenant_id=${tenant.slug}`); setReservations(await r.json()); } catch(e) { console.error(e); }
  };

  const fetchDraft = async () => {
    try {
      const r = await fetch(`/api/orders?type=draft&tenant_id=${tenant.slug}`);
  ...
  useEffect(() => {
    if (isAuthenticated && isDraftLoaded) {
      localStorage.setItem(`deportux_draft_order_${tenant.slug}`, JSON.stringify(activeOrderItems));
      const timer = setTimeout(() => {
        fetch(`/api/orders?type=draft&tenant_id=${tenant.slug}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  }, [activeOrderItems, isAuthenticated, isDraftLoaded, tenant.slug]);
  ...
  const handleCatalogSubmit = async (e) => {
    ...
    try {
      ...
      const pResp = await fetch(`/api/products?tenant_id=${tenant.slug}`, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  ...
  const resetForm = () => {
    setNewProduct({ name: '', size: '', price: '', type: 'stock', category: tenant.categories[0]?.name || 'Adulto', is_favorite: false, image: null, stock_quantity: 0, stock_by_size: {} });
  ...
  const handleEdit = (p) => {
    setNewProduct({ 
      ...
      category: p.category || tenant.categories[0]?.name || 'Adulto', is_favorite: p.is_favorite || false, 
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Eliminar producto?')) {
      try {
        const resp = await fetch(`/api/products?id=${id}&tenant_id=${tenant.slug}`, { method: 'DELETE' });
  ...
  const addToOrderList = (p) => {
    ...
    setActiveOrderItems([...activeOrderItems, {
      ...
      image_url: p.image_url, quantity: 1, comment: '', category: p.category || tenant.categories[0]?.name || 'Adulto',
  ...
  const updateOrderItem = (orderId, updates) => {
    ...
    if (updates.size && updates.size !== newItems[idx].size) {
      const baseProduct = products.find(p => p.name === item.name && (p.category || tenant.categories[0]?.name) === (item.category || tenant.categories[0]?.name));
      if (baseProduct) {
  ...
  const handleFinalizeOrder = async () => {
    ...
    try {
      ...
      const orderResp = await fetch(`/api/orders?tenant_id=${tenant.slug}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  const toggleOrderItemReceived = async (order, itemIdx) => {
    ...
    // 2. Background sync
    try {
      await fetch(`/api/orders?tenant_id=${tenant.slug}`, {
  ...
  const handleReturnSale = async (id) => {
    ...
    setIsUploading(true);
    try {
      await fetch(`/api/sales?id=${id}&tenant_id=${tenant.slug}`, { method: 'DELETE' });
  ...
  const handleReassignSale = async (saleId, newCustomerId) => {
    ...
    setIsUploading(true);
    try {
      await fetch(`/api/sales?tenant_id=${tenant.slug}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleReceiveIntoInventory = async (order) => {
    ...
    try {
      ...
      for (const resItem of reservationsToCreate) {
        await fetch(`/api/reservations?tenant_id=${tenant.slug}`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      for (const [prodId, data] of updatesMap.entries()) {
        ...
        await fetch(`/api/products?tenant_id=${tenant.slug}`, {
          method: 'PUT', headers: { 'Content-Type': 'application/json' },
  ...
  const addToSaleList = (p) => {
    ...
    setActiveSaleItems([...activeSaleItems, {
      ...
      size: defaultSize || '', image_url: p.image_url, quantity: 1, category: p.category || tenant.categories[0]?.name || 'Adulto',
      stock_by_size: p.stock_by_size || {}
    }]);
    setIsSaleOpen(true);
  };
  ...
  const handleDeleteCustomer = async (id) => {
    ...
    setIsUploading(true);
    try {
      await fetch(`/api/customers?id=${id}&tenant_id=${tenant.slug}`, { method: 'DELETE' });
      fetchCustomers();
    } catch (e) { alert(e.message); } finally { setIsUploading(false); }
  };
        ...
        {activeTab === 'inventory' && (
          <div className="admin-main-grid">
            <div className="admin-content-area">
              <section className="glass" style={{ padding: '2rem', marginBottom: '2rem' }}>
                ...
                <form onSubmit={handleCatalogSubmit} className="admin-form-grid">
                  ...
                  <div className="admin-form-row">
                    ...
                    >
                      {tenant.categories.map(c => <option key={c.id || c.name} value={c.name}>{c.name}</option>)}
                    </select>
                  </div>
                  <div className="admin-form-row">
              ...
              {tenant.categories.map(c => c.name).map(cat => (
                <div key={cat} style={{ marginBottom: '1.5rem' }}>
                  <div onClick={() => setOpenCategory(openCategory === cat ? null : cat)} style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem', background: 'rgba(255,255,255,0.05)', borderRadius: '12px', cursor: 'pointer', marginBottom: '1rem' }}>
                    <h4 style={{ margin: 0 }}>{cat.toUpperCase()} ({products.filter(p => {
                      const matchesCategory = (p.category || tenant.categories[0]?.name) === cat;
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
              <div className="glass" style={{ gridColumn: '1 / -1', padding: '1.5rem' }}>
                ...
                <form onSubmit={async e => { 
                  e.preventDefault(); 
                  await fetch(`/api/customers?tenant_id=${tenant.slug}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newCustomer) }); 
          ...
          <div className="glass" style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2.5rem' }}>
              {tenant.categories.map((cat, i) => {
                ...
                const sizes = availableSizes[catName] || [];

                return (
                  <section key={catName}>
                    <h4 style={{ marginBottom: '1rem', color }}>📏 Tallas {catName}</h4>
                    <div className="size-selector-grid" style={{ marginBottom: '1.5rem' }}>
                      {sizes.map(size => (
                        <div key={`${catName}-${size}`} style={{ position: 'relative', display: 'inline-block' }}>
                          <span className="size-tag active" style={{ paddingRight: '2.5rem', background: color, borderColor: color }}>{size}</span>
                          <button 
                            onClick={() => setAvailableSizes(prev => ({ ...prev, [catName]: prev[catName].filter(s => s !== size) }))}
                    ...
                    <form 
                      onSubmit={e => {
                        ...
                        if (val && !sizes.includes(val)) {
                          setAvailableSizes(prev => ({ ...prev, [catName]: [...(prev[catName] || []), val] }));
                    ...
                    >
                      ...
                      <button type="submit" className="btn btn-primary" style={{ padding: '0.5rem 1rem', background: color }}>+</button>
                    </form>
                  </section>
                );
              })}

              <div style={{ gridColumn: '1 / -1', padding: '1rem', background: 'rgba(239, 129, 30, 0.1)', border: '1px solid var(--primary)', borderRadius: '12px', fontSize: '0.85rem', marginTop: '1rem' }}>
